...
@onready var health_add_sound: AudioStreamPlayer2D = %HealthAddSound
...
func play_skater_rolling_sound(skater: Skater)-> void:
	...
	if GameStateManager.current_state != GameStateManager.State.PLAYING:
		self.skater_rolling_sound.volume_linear = 0.0
		return
	if skater._is_on_ground(): #Hmm, eine private Methode aufrufen...
...
func play_health_add_sound() -> void:
	self.health_add_sound.play()
